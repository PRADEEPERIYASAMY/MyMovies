...
fun Activity.hideBottomNav() {
    findViewById<BottomNavigationView>(R.id.bottom_nav)?.gone()
}

fun Activity.showBottomNav() {
    findViewById<BottomNavigationView>(R.id.bottom_nav)?.show()
}
...
fun ImageView.downloadImage(image: String?, addPlaceHolder: Boolean = false) {
    Picasso.get().load(image).apply {
        if (addPlaceHolder)
            placeholder(R.drawable.ic_launcher_background).into(this@downloadImage)
        else
